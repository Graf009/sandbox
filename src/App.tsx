...
const stationReducer = (state: Station, event: AnyDispatchedEvent): Station => {
  if (state.service) {
    ...
    const next = state.service.send({...event, __storeUpdate: true });
...
const useSupervisedMachine = <
  TContext,
  TEvent extends X.EventObject & { __storeUpdate?: true } = X.AnyEventObject,
...
>(
  ...
  const [state, , service] = useMachine<TContext, TEvent>(machine, { state: maybeStation.state} );
  ...
  React.useEffect(() => {
    ...
    const subscription = service.subscribe(nextState => {      
      if (!nextState._event.data.__storeUpdate) {
        if (store.getState()[id].state !== nextState) {
          
          dispatch({
            type: "REGISTER_MACHINE",
            // @ts-ignore
            payload: {...maybeStation, state: nextState}
          });
        }
      }
    })
 ...
 * 
 */
const Input = ({ id, onChange, disabled }: PropsInput) => {
...
const formM = createFormMachine('form')
...
export default () => {
  ...
  const [formState, formSend] = useSupervisedMachine(formM)

  if (formState.matches('done')) {
    return <div>
    {formState.context.value}
    <div>
    <input type="button" value="Reset" onClick={() => formSend([
      { type: 'CHANGE', to: ['City', 'Province', 'State', 'Country'] },
      'RESET',
    ])} />      
    </div>
    </div>
  ...
  // Flag used to disable form controls while submitting
  const isSubmitting = formState.matches('submitting')

  // Element displaying a error message of the form state's context has one
  const errorEl = 
    formState.matches('editing') && !!formState.context.error
      ? <div style={{color: 'red'}}>{formState.context.error}</div>
      : null
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    formSend({ type: 'SUBMIT', task: () => Promise.resolve("Thank you!")})
  }

  const onSubmitButFail = () => {
    formSend({ type: 'SUBMIT', task: () => Promise.reject("Oops!")})
  }

  const onChangeCountryEvents = () => [{ type: "CHANGE", value: "", to: "City" }]
  ...
  return (
    <Provider store={store}>
      ...
      <Input
        ...
        onChange={onChangeCountryEvents}
      />
      <br />
      <input
        disabled={isSubmitting}
      ...
      <br />
      <br />
      <Input disabled={isSubmitting} id="City" />
      <br />
      <br />
      <hr />
      <br />
      ...
      {' '}
      <input disabled={isSubmitting} type="button" value="Submit but fail" onClick={onSubmitButFail} />
      </form>
    </Provider>
  );
};
